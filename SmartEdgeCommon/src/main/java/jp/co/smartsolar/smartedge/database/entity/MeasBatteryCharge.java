...
/**
 * The persistent class for the meas_battery_charge database table.
 *
 */
@Entity
@Table(name = "meas_battery_charge")
@NamedQuery(name = "MeasBatteryCharge.findAll", query = "SELECT m FROM MeasBatteryCharge m")
@NamedQuery(name = "MeasBatteryCharge.deleteOldData", query = "DELETE FROM MeasBatteryCharge m WHERE m.cycleDatetime <= :oldDate")
@NamedQuery(name = "MeasBatteryCharge.getLatestData", query = "SELECT m FROM MeasBatteryCharge m ORDER BY m.cycleDatetime DESC ")
@NamedQuery(name = "MeasBatteryCharge.findByDeviceMasterId", query = "SELECT m FROM MeasBatteryCharge m WHERE m.deviceMaster.id = :id")
public class MeasBatteryCharge implements Serializable {
